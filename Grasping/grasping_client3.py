...
def get_grasps(self, colors, depths, prompt, traj_length=0.2, traj_radius=0.05):
    ...
    Utils.print_debug(f"Grasp Poses: {grasp_poses}", self.args.debug)
    Utils.print_debug(f"Grasp Scores: {grasp_scores}", self.args.debug)
    Utils.print_debug(f"Grasp Widths: {grasp_widths}", self.args.debug)
    Utils.print_debug(f"LangSAM Mask: {langsam_masks}", self.args.debug)
